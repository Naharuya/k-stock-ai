...
if($('screenerBody')) $('screenerBody').addEventListener('click',e=>{const b=e.target.closest('.screener-analyze');if(!b)return;selectedStockCode=b.dataset.code;$('stockCode').value=b.dataset.name;window.scrollTo({top:0,behavior:'smooth'});analyze();});
...
async function runTodayCandidates(){
  ...
  try{
    ...
    $('candidateBody').innerHTML=rows.length?rows.map((c,i)=>{const [vl,vc,grade]=verifyLabel(c);const tags=(c.styleTags||[]).slice(0,3);const cs=committeeLabel(c);const st=c.committee?.status||'';const cclass=c.committee?.hardStop||st==='RISK'?'risk':(['INTEREST','CONDITION_MET'].includes(st)?'interest':st==='WATCH'?'watch':'');return `<tr>
      ...
      <td><button class="screener-analyze candidate-analyze" data-code="${c.code}" data-name="${escapeHtml(c.name)}">개별분석</button></td></tr>`}).join(''):'<tr><td colspan="10" class="empty">후보가 없습니다.</td></tr>';
...
function candidateAnalyzeClick(e){const b=e.target.closest('.candidate-analyze');if(!b)return;selectedStockCode=b.dataset.code;$('stockCode').value=b.dataset.code;window.scrollTo({top:0,behavior:'smooth'});analyze();}
...
function positionPnl(entry,last){
  const cp=Number(last?.currentPrice); const bp=Number(entry?.buyPrice); if(!Number.isFinite(cp)||!Number.isFinite(bp)||bp<=0)return null; return ((cp-bp)/bp)*100;
}
...
async function saveCurrentEntry(){
  ...
  try{
    ...
    let rows=loadPositions(); const idx=rows.findIndex(x=>x.entry?.stockCode===b.result.stockCode); const record={entry:b.result,lastEvaluation:null};
...
if($('portfolioList')) $('portfolioList').addEventListener('click',e=>{
  ...
  const del=e.target.closest('.position-delete'); if(del){const rows=loadPositions();rows.splice(Number(del.dataset.index),1);savePositions(rows);renderPortfolio();return;}
