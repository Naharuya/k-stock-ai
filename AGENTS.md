...
PROJECT=K-STOCK-AI
TYPE=Node.js + Agents + KIS + OpenDART
...
## 프로젝트 규칙
- LIVE_TRADING=false를 기본으로 유지한다.
- 실제 주문은 자동 실행하지 않는다. 실거래는 별도 명시적 승인 전까지 금지한다.
- 뉴스/데이터 출처와 관측 시각을 구분한다. 추정값을 실제 데이터로 표시하지 않는다.
- 403/429/EGW 오류의 원인별 retry 정책과 제한을 검증한다. 무한 재시도하지 않는다.
- cache/token 보안을 유지한다.
- 투자 조언을 확정적으로 표현하지 않는다.
- ONARIA/RAMI를 수정하지 않는다.
- package.json 및 기존 CI의 테스트 명령을 재사용한다. 기본 검증은 mock/offline으로 하고 실제 주문을 테스트로 사용하지 않는다.
