...
## 핵심 오류 수정 (2026-09-12)
- 필수 데이터가 누락되면 Exit 및 종합점수 판정을 보류합니다.
- 보유종목 삭제·재등록·동시 재검증 시 오래된 응답이 최신 목록을 덮어쓰지 않습니다.
- 뉴스 해제·부인 문맥을 구분하고 제목 키워드만으로 Hard Stop을 발동하지 않습니다. 미확인 위험은 추가 확인 대상으로 남기며 공시·리스크 Hard Stop은 유지합니다.
- `npm ci` 후 `npm test`로 오프라인 회귀 검사를 실행합니다.
