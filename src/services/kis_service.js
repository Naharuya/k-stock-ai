...
async function readPersistentToken() {
  try {
    ...
    if (error?.code !== "ENOENT") {
      console.warn(`[KIS] token cache read skipped: ${error instanceof SyntaxError ? "INVALID_JSON" : "READ_FAILED"}`);
    }
  }
...
export async function getKisAccessToken({ force = false } = {}) {
  ...
  const response = await providerFetch(`${baseUrl()}/oauth2/tokenP`, {
    method: "POST",
    headers: {
  ...
  try {
    ...
    console.warn("[KIS] token cache write skipped: WRITE_FAILED");
  }

  return cachedToken;
}

export function kisRequestPolicy(env=process.env) {
  const bounded=(raw,fallback,min,max,integer=false)=>{
    const value=raw===undefined||String(raw).trim()===''?fallback:Number(raw);
    if(!Number.isFinite(value)||value<0||(integer&&!Number.isInteger(value)))return fallback;
    return Math.max(min,Math.min(value,max));
  };
  return {
    minIntervalMs:bounded(env.KIS_API_MIN_INTERVAL_MS,1200,1200,60000),
    maxRetries:bounded(env.KIS_RATE_LIMIT_MAX_RETRIES,2,0,5,true),
    retryMs:bounded(env.KIS_RATE_LIMIT_RETRY_MS,61000,1000,300000)
  };
}

async function waitForApiSlot() {
  const waitMs = Math.max(0, kisRequestPolicy().minIntervalMs - (Date.now() - lastApiRequestAt));
...
async function kisGet(pathname, trId, params) {
  return enqueueApiRequest(async () => {
    const {maxRetries,retryMs} = kisRequestPolicy();
...
export async function getDomesticQuote(stockCode) {
  ...
  return {
    ...
    tradeDate:o.stck_bsop_date||null,
