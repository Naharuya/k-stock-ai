...
export function buildInvestmentCommittee({ dart, valuation, technical, flow, market, news, completeness, bear: suppliedBear }) {
  const weights = {
    fundamental: 25,
    valuation: 15,
    technical: 15,
  ...
  const bear = suppliedBear || buildBearCase({ dart, valuation, technical, flow, market, news });

  const criticalNews = Array.isArray(news?.highRiskEvents) && news.highRiskEvents.some(event => event.verified === true && event.evidenceSource === 'PRIMARY');
