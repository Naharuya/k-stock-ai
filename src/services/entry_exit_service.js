...
function getCurrentPrice(analysis) {
  const value=num(analysis?.marketData?.quote?.price ?? analysis?.marketData?.quote?.currentPrice);
...
export function evaluateExitPosition({ entry, currentAnalysis }) {
  ...
  exitPressure = currentPrice === null ? null : clamp(exitPressure);

  let status = currentPrice === null ? "WATCH" : "HOLD";
