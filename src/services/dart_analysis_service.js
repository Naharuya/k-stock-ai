import {getLatestFinancials} from "./latest_financials_service.js";
import {
...
export async function analyzeDartStock({
  ...
  const latestFinancials=latest?(String(companyResult.company.acc_mt)==='12'?await getLatestFinancials({stockCode,asOf,fsDiv}):{status:'NOT_AVAILABLE',reason:'NON_DECEMBER_FISCAL_YEAR'}):null;
  const current = normalizedFinancials.current;

  return {
    latestFinancials,
    scoreBasis: "ANNUAL_RULE_MODEL",
    source: "OpenDART",
