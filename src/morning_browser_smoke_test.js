...
try {
  ...
  for(const [name,width,height,mobile] of [['desktop',1440,1000,false],['mobile',390,844,true]]){
    ...
    assert.equal(await evaluate("document.getElementById('morningBasis').textContent.includes('\uD734\uC7A5\uC77C')"),true,'Closed-session basis must be visible');
    const briefOrder=await evaluate("['morningCandidates','morningRisk','morningPortfolio','dailyTomorrowCards','dailyActions'].map(id=>document.getElementById(id).getBoundingClientRect().top)");
    assert.ok(briefOrder.every((y,i)=>i===0||y>briefOrder[i-1]),'Morning cards, risk, Exit, tomorrow and manual controls must remain in priority order');
    assert.equal(requests.length,0,'Opening the page must not start a scan');
    const screenshot=await send('Page.captureScreenshot',{format:'png',captureBeyondViewport:false});
    await fs.writeFile(path.resolve('.cache/morning-dashboard-'+name+'.png'),Buffer.from(screenshot.data,'base64'));
  }
  ...
  console.log('Morning Brief browser: read-only initial load, scoped Exit button, first-screen brief, desktop 1440px / mobile 390px; five saved cards, dynamic version, mobile order, no horizontal overflow, no JS exceptions passed (fixture data)');
} finally {
  socket?.close();
